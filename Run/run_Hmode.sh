...
cp input_Hmode $out
cp wout_Hmode.nc $out
cp ../PB3D $out
chmod +x $out/PB3D
cd $out
rm -f .lock_file*
echo "$debug_opt -- mpirun -np $1 ./PB3D input_Hmode wout_Hmode.nc $slepc_opt ${@:2}" > command
$debug_opt mpirun -np $1 ./PB3D input_Hmode wout_Hmode.nc $slepc_opt ${@:2}
cd ../
echo ""
