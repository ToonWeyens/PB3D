...
create_loc_script() {
    ...
    cat <<- END > $prog_name"_loc.sh"
        ...
        # run mpi command
        run=$(which mpirun)
        echo "$(echo $debug_opt \$run | xargs) -np $nr_procs \$MPI_command" > command_$prog_name
        . command_$prog_name
...
# sets: PB3D:   PB3D.pbs
#       POST:   POST.pbs
#for automatic node selection: #PBS -l nodes=$n_nodes:ppn=$n_cores
#for manual node selection:    #PBS -l nodes=$node_list
setup_pbs_script_1() {
    # print into script
