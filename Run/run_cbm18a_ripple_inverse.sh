...
cp wout_cbm18a_ripple_inverse.nc $out
cp ../PB3D $out
chmod +x $out/PB3D
cd $out
rm -f .lock_file*
echo "$debug_opt -- mpirun -np $1 ./PB3D input_cbm18a_ripple wout_cbm18a_ripple_inverse.nc $slepc_opt ${@:2}" > command
$debug_opt mpirun -np $1 ./PB3D input_cbm18a_ripple wout_cbm18a_ripple_inverse.nc $slepc_opt ${@:2}
cd ../
echo ""
