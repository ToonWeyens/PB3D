...
#include <wrappers.h>
    ...
    implicit none
    private
    public calc_XUQ, calc_tot_sol_vec, calc_loc_sol_vec, calc_pert_cart_comp
#if ldebug
    ...
    interface calc_pert_cart_comp
        module procedure calc_pert_cart_comp_arr, calc_pert_cart_comp_ind
    end interface
...
contains
    ...
    integer function calc_pert_cart_comp_arr(grid_eq,grid_X,eq_1,eq_2,X,sol,&
        ...
        type(sol_type), intent(in) :: sol                                       ! solution variables
        integer, intent(in) :: X_id                                             ! nr. of Eigenvalue
        integer, intent(in) :: pert_style                                       ! whether to calculate plasma (1) or magn. (2) perturbation
        real(dp), intent(in) :: time(:)                                         ! time range
        real(dp), intent(inout) :: ccomp(:,:,:,:,:)                             ! Cart. components of perturbation
        
        ! local variables
        integer :: it, id, jd                                                   ! counters
        character(len=max_str_ln) :: err_msg                                    ! error message
        complex(dp), allocatable :: ccomp_F(:,:,:,:,:)                          ! Cart. components of perturbation in Flux coords. (contravar.)
        real(dp), allocatable :: T_FE_int(:,:,:,:)                              ! interpolated T_FE
        real(dp), allocatable :: T_VC_int(:,:,:,:)                              ! interpolated T_VC
        real(dp), allocatable :: h_FD_int(:,:,:,:)                              ! interpolated h_FD
        ...
        ! interpolate
        allocate(T_FE_int(grid_X%n(1),grid_X%n(2),grid_X%loc_n_r,&
            &size(eq_2%T_FE,4)))
        allocate(T_VC_int(grid_X%n(1),grid_X%n(2),grid_X%loc_n_r,&
            &size(eq_2%T_VC,4)))
        allocate(h_FD_int(grid_X%n(1),grid_X%n(2),grid_X%loc_n_r,&
            &size(eq_2%h_FD,4)))
        allocate(g_FD_int(grid_X%n(1),grid_X%n(2),grid_X%loc_n_r,&
            &size(eq_2%g_FD,4)))
        allocate(R_int(grid_X%n(1),grid_X%n(2),grid_X%loc_n_r))
        ierr = apply_disc(eq_2%T_FE(:,:,:,:,0,0,0),norm_interp_data,T_FE_int,3)
        CHCKERR('')
        ierr = apply_disc(eq_2%T_VC(:,:,:,:,0,0,0),norm_interp_data,T_VC_int,3)
        CHCKERR('')
        ierr = apply_disc(eq_2%h_FD(:,:,:,:,0,0,0),norm_interp_data,h_FD_int,3)
        CHCKERR('')
        ierr = apply_disc(eq_2%g_FD(:,:,:,:,0,0,0),norm_interp_data,g_FD_int,3)
        CHCKERR('')
        ierr = apply_disc((eq_2%g_E(:,:,:,c([3,3],.true.),0,0,0))**(-0.5),&
            &norm_interp_data,R_int,3)
        CHCKERR('')
        
        ! clean up normal interpolation data
        call norm_interp_data%dealloc()
        
        ! Calculate the required components in the Flux components:
        !   xi . nabla alpha = X h12/h22 + U
        !   xi . nabla psi   = X
        !   xi . nabla theta = X h23/h22 - U g13/g33
        ! and similarly for Q.
        allocate(ccomp_F(size(ccomp,1),size(ccomp,2),size(ccomp,3),&
            &size(ccomp,4),3))
        select case (pert_style)
            case (1)                                                            ! plasma perturbation
                ierr = calc_XUQ_arr(grid_eq,grid_X,eq_1,eq_2,X,sol,X_id,&
                    &1,time,ccomp_F(:,:,:,:,2))
                CHCKERR('')
                ierr = calc_XUQ_arr(grid_eq,grid_X,eq_1,eq_2,X,sol,X_id,&
                    &2,time,ccomp_F(:,:,:,:,1))
            ...
            case (2)                                                            ! magnetic perturbation
                ...
                CHCKERR('')
            case default
                err_msg = 'No perturbation style associated with '//&
                    &trim(i2str(pert_style))
                ierr = 1
                CHCKERR(err_msg)
        end select
        ! convert to contravariant components in the Flux coordinates
        do it = 1,size(time)
            ccomp_F(:,:,:,it,3) = ccomp_F(:,:,:,it,2) * &
                &h_FD_int(:,:,:,c([3,2],.true.))/&
                &h_FD_int(:,:,:,c([2,2],.true.)) - &
                &ccomp_F(:,:,:,it,1) * &
                &g_FD_int(:,:,:,c([3,1],.true.))/&
                &g_FD_int(:,:,:,c([3,3],.true.)) 
            ccomp_F(:,:,:,it,1) = ccomp_F(:,:,:,it,2) * &
                &h_FD_int(:,:,:,c([1,2],.true.))/&
                &h_FD_int(:,:,:,c([2,2],.true.)) + &
                &ccomp_F(:,:,:,it,1)
        end do
        
        ! convert to real cylindrical components:
        !   xi = ((xi^i)_F)^T (e_i)_F
        !      = ((xi^i)_F)^T T_F^E T_E^C (e_i)_C
        ccomp = 0._dp
        do id = 1,3
            do jd = 1,3
                do it = 1,size(time)
                    ccomp(:,:,:,it,id) = ccomp(:,:,:,it,id) + &
                        &rp(ccomp_F(:,:,:,it,jd)) * &
                        &T_FE_int(:,:,:,c([jd,id],.false.))
                end do
            end do
        end do
        ccomp_F = ccomp
        ...
        !    (e_Z)    (     0          0       1 ) (e_Z)
        ! with phi = zeta_E = - zeta_F
        ccomp_F = ccomp
        ccomp = 0._dp
        do it = 1,size(time)
            ccomp(:,:,:,it,1) = rp(ccomp_F(:,:,:,it,1))*cos(grid_X%zeta_E) + &
                &rp(ccomp_F(:,:,:,it,2))*sin(grid_X%zeta_E)
            ccomp(:,:,:,it,2) = R_int*&
                &(-rp(ccomp_F(:,:,:,it,1))*sin(grid_X%zeta_E) + &
        ...
        ! clean up
        deallocate(ccomp_F)
    end function calc_pert_cart_comp_arr
    integer function calc_pert_cart_comp_ind(grid_eq,grid_X,eq_1,eq_2,X,sol,&
        &X_id,pert_style,time,ccomp) result(ierr)                               ! (time) individual version
        character(*), parameter :: rout_name = 'calc_pert_cart_comp_ind'
        ...
        type(sol_type), intent(in) :: sol                                       ! solution variables
        integer, intent(in) :: X_id                                             ! nr. of Eigenvalue
        integer, intent(in) :: pert_style                                       ! whether to calculate X, U, Qn or Qg
        real(dp), intent(in) :: time                                            ! time
        real(dp), intent(inout) :: ccomp(:,:,:,:)                               ! normal component of perturbation
        
        ! local variables
        real(dp), allocatable :: ccomp_arr(:,:,:,:,:)                           ! dummy variable to use array version
        
        ! allocate array version of ccomp
        allocate(ccomp_arr(size(ccomp,1),size(ccomp,2),size(ccomp,3),1,3))
        
        ! call array version
        ierr = calc_pert_cart_comp_arr(grid_eq,grid_X,eq_1,eq_2,X,sol,X_id,&
            &pert_style,[time],ccomp_arr)
        CHCKERR('')
        
        ! copy array to individual ccomp
        ccomp = ccomp_arr(:,:,:,1,:)
        
        ! deallocate array version of ccomp
        deallocate(ccomp_arr)
    end function calc_pert_cart_comp_ind
