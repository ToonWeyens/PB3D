...
contains
    ...
    ! coordinate systems:
    !   - F: (alpha,psi,theta),
    !   - H: (psi,theta,phi) if HELENA is used,
    ...
    ! Optionally, the Richardson level can be appended to the file names.
    ...
    integer function B_plot(grid_eq,eq,rich_lvl) result(ierr)
        ...
        character(len=5) :: par_coord_name                                      ! name of parallel coordinate
        ...
        real(dp), allocatable :: B_com(:,:,:,:,:)                               ! covariant and contravariant components of B
        real(dp), allocatable :: B_mag(:,:,:)                                   ! magnitude of B
        ...
        ! calculate XYZR of untrimmed grid, copy to trimmed grid copies
        ...
        allocate(B_com(grid_trim%n(1),grid_trim%n(2),grid_trim%loc_n_r,3,2))
        allocate(B_mag(grid_trim%n(1),grid_trim%n(2),grid_trim%loc_n_r))
        allocate(B_temp(grid_trim%n(1),grid_trim%n(2),grid_trim%loc_n_r,3,2))
        allocate(T_BA(grid_trim%n(1),grid_trim%n(2),grid_trim%loc_n_r,9,&
            &0:0,0:0,0:0))
        allocate(T_AB(grid_trim%n(1),grid_trim%n(2),grid_trim%loc_n_r,9,&
            &0:0,0:0,0:0))
        
        ! set up plot variables
        cont_plot = eq_job_nr.gt.1
        if (use_pol_flux_F) then
            par_coord_name = 'theta'
        else
            par_coord_name = 'zeta'
        end if
        var_names = 'B'
        do id = 1,3
            var_names(id,1) = trim(var_names(id,1))//'_sub_'//trim(i2str(id))
            var_names(id,2) = trim(var_names(id,2))//'_sup_'//trim(i2str(id))
        end do
        ...
        B_mag = 0._dp
        do id = 1,3
            B_com(:,:,:,id,1) = eq%g_FD(:,:,norm_id(1):norm_id(2),&
                &c([3,id],.true.),0,0,0)/&
                &eq%jac_FD(:,:,norm_id(1):norm_id(2),0,0,0)
            B_com(:,:,:,3,2) = 1._dp/eq%jac_FD(:,:,norm_id(1):norm_id(2),0,0,0)
            B_mag = B_mag + B_com(:,:,:,id,1)*B_com(:,:,:,id,2)
        end do
        B_mag = sqrt(B_mag)
        
        description(1) = 'covariant components of the magnetic field in &
            &Flux coordinates (alpha,psi,'//trim(par_coord_name)//')'
        description(2) = 'contravariant components of the magnetic field in &
            &Flux coordinates (alpha,psi,'//trim(par_coord_name)//')'
        description(3) = 'magnitude of the magnetic field in Flux coordinates &
            &(alpha,psi,'//trim(par_coord_name)//')'
        file_names(1) = 'B_F_sub'
        file_names(2) = 'B_F_sup'
        file_names(3) = 'B_F_mag'
        do id = 1,2
            call plot_HDF5(var_names(:,id),trim(file_names(id))//&
                &trim(rich_name(rich_lvl)),B_com(:,:,:,:,id),tot_dim=plot_dim,&
                &loc_offset=plot_offset,X=X,Y=Y,Z=Z,cont_plot=cont_plot,&
                &description=description(id))
        end do
        call plot_HDF5('B',trim(file_names(3))//trim(rich_name(rich_lvl)),&
            &B_mag,tot_dim=plot_dim(1:3),loc_offset=plot_offset(1:3),&
            &X=X(:,:,:,1),Y=Y(:,:,:,1),Z=Z(:,:,:,1),&
        ...
        do jd = 1,3
            do id = 1,3
                B_com(:,:,:,jd,1) = B_com(:,:,:,jd,1) + &
                    &eq%T_EF(:,:,norm_id(1):norm_id(2),&
                    &c([jd,id],.false.),0,0,0) * B_temp(:,:,:,id,1)
                B_com(:,:,:,jd,2) = B_com(:,:,:,jd,2) + &
                    &eq%T_FE(:,:,norm_id(1):norm_id(2),&
        ...
        select case (eq_style)
            case (1)                                                            ! VMEC
                description(1) = 'covariant components of the magnetic field &
                    &in VMEC coordinates (r,theta,phi)'
                description(2) = 'contravariant components of the magnetic &
                    &field in VMEC coordinates (r,theta,phi)'
                description(3) = 'magnitude of the magnetic field in VMEC &
                    &coordinates (r,theta,phi)'
                file_names(1) = 'B_V_sub'
                file_names(2) = 'B_V_sup'
                file_names(3) = 'B_V_mag'
#if ldebug
                if (ltest) then
                    call writo('Use the VMEC output for covariant comp. and &
                        &magnitude of  B?')
                    if(get_log(.false.)) then
                        ierr = B_plot_VMEC(B_com(:,:,:,:,1),B_mag)
                        CHCKERR('')
                        file_names(1) = trim(file_names(1))//'_VMEC'
                        file_names(2) = trim(file_names(2))//'_VMEC'
                        file_names(3) = trim(file_names(3))//'_VMEC'
                    end if
                end if
#endif
            case (2)                                                            ! HELENA
                description(1) = 'covariant components of the magnetic field &
                    &in HELENA coordinates (psi,theta,phi)'
                description(2) = 'contravariant components of the magnetic &
                    &field in HELENA coordinates (psi,theta,phi)'
                description(3) = 'magnitude of the magnetic field in HELENA &
                    &coordinates (psi,theta,phi)'
                file_names(1) = 'B_H_sub'
                file_names(2) = 'B_H_sup'
                file_names(3) = 'B_H_mag'
        end select
        do id = 1,2
            call plot_HDF5(var_names(:,id),trim(file_names(id))//&
                &trim(rich_name(rich_lvl)),B_com(:,:,:,:,id),tot_dim=plot_dim,&
        ...
        select case (eq_style)
            case (1)                                                            ! VMEC
                T_AB = eq%T_VC(:,:,norm_id(1):norm_id(2),:,0:0,0:0,0:0)
            case (2)                                                            ! HELENA
                ...
                T_AB(:,:,:,c([1,1],.false.),0,0,0) = &
                    &D1R(:,:,norm_id(1):norm_id(2))
                T_AB(:,:,:,c([1,3],.false.),0,0,0) = &
                    &D1Z(:,:,norm_id(1):norm_id(2))
                T_AB(:,:,:,c([2,1],.false.),0,0,0) = &
                    &D2R(:,:,norm_id(1):norm_id(2))
                T_AB(:,:,:,c([2,3],.false.),0,0,0) = &
                    &D2Z(:,:,norm_id(1):norm_id(2))
        ...
        description(3) = 'magnitude of the magnetic field in cylindrical &
            &coordinates (R,phi,Z)'
        file_names(1) = 'B_C_sub'
        file_names(2) = 'B_C_sup'
        file_names(3) = 'B_C_mag'
        do id = 1,2
            call plot_HDF5(var_names(:,id),trim(file_names(id))//&
                &trim(rich_name(rich_lvl)),B_com(:,:,:,:,id),tot_dim=plot_dim,&
                &loc_offset=plot_offset,X=X,Y=Y,Z=Z,cont_plot=cont_plot,&
        ...
        !    (e_Z)    (     0          0       1 ) (e_Z)
        ! with phi = zeta_E = -  zeta_F. For the transformation of contravariant
        ! components, the factor R becomes 1/R and the transpose is taken.
        T_BA = 0._dp
        T_AB = 0._dp
        T_AB(:,:,:,c([1,1],.false.),0,0,0) = cos(grid_trim%zeta_E)
        T_AB(:,:,:,c([1,2],.false.),0,0,0) = sin(grid_trim%zeta_E)
        T_AB(:,:,:,c([2,1],.false.),0,0,0) = &
            &-XYZR(:,:,norm_id(1):norm_id(2),4)/R_0*sin(grid_trim%zeta_E)
        T_AB(:,:,:,c([2,2],.false.),0,0,0) = &
            &XYZR(:,:,norm_id(1):norm_id(2),4)/R_0*cos(grid_trim%zeta_E)
        ...
        description(3) = 'magnitude of the magnetic field in cartesian &
            &coordinates (X,Y,Z)'
        file_names(1) = 'B_X_sub'
        file_names(2) = 'B_X_sup'
        file_names(3) = 'B_X_mag'
        do id = 1,2
            call plot_HDF5(var_names(:,id),trim(file_names(id))//&
                &trim(rich_name(rich_lvl)),B_com(:,:,:,:,id),tot_dim=plot_dim,&
                &loc_offset=plot_offset,X=X,Y=Y,Z=Z,cont_plot=cont_plot,&
        ...
        ! plot vector as well
        call plot_HDF5(['B'],'B_vec'//trim(rich_name(rich_lvl)),&
            &B_com(:,:,:,:,1),tot_dim=plot_dim,loc_offset=plot_offset,&
            &X=X,Y=Y,Z=Z,col=4,cont_plot=cont_plot,&
            &description='magnetic field vector')
    contains
        ! adds "R_" to file name or not
        character(len=max_str_ln) function rich_name(rich_lvl)
            ! input/output
            integer, intent(in), optional :: rich_lvl                           ! Richardson level
            
            rich_name = ''
            if (present(rich_lvl)) then
                if (rich_lvl.gt.0) rich_name = '_R_'//trim(i2str(rich_lvl))
            end if
        end function rich_name
#if ldebug
        ! Calculate with VMEC output
        integer function B_plot_VMEC(B_V_com,B_V_mag) result(ierr)
            use VMEC, only: fourier2real, &
                &B_V_sub_s, B_V_sub_c, B_V_c, B_V_s, is_asym_V
            
            character(*), parameter :: rout_name = 'B_plot_VMEC'
            
            ! input / output
            real(dp), intent(inout) :: B_V_com(:,:,:,:)                         ! VMEC components of B
            real(dp), intent(inout) :: B_V_mag(:,:,:)                           ! VMEC magnitude of B
            ...
            ! calculate 
                do id = 1,3
                    ierr = fourier2real(&
                        ...
                        &grid_trim%trigon_factors,B_V_com(:,:,:,id))
                    CHCKERR('')
                end do
                ierr = fourier2real(&
                    &B_V_c(:,norm_id_f(1):norm_id_f(2)),&
                    &B_V_s(:,norm_id_f(1):norm_id_f(2)),&
                    &grid_eq%trigon_factors,B_V_mag,&
                    &sym=[.true.,is_asym_V])
                CHCKERR('')
        end function B_plot_VMEC
#endif
    end function 
