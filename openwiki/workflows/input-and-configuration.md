...
| `BC_style(2)` | Boundary-condition style at the two radial boundaries: 1 = eigenvector zeroed (fixed boundary); 2 = variational imposition of the natural (surface-energy-minimizing) boundary condition with the vacuum response — keeps A and B Hermitian, numerically the cleanest free-boundary choice; 4 = explicit natural-BC row (the historical free-boundary style and current default; agrees with 2 to 8 digits on the cbm18a benchmark). Style 3 was removed |
