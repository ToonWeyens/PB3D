...
- **The vacuum module (`vac_ops.f90`) is partially verified**: the axisymmetric building blocks (kernels, assembled G/H matrices, boundary potential solve) are covered by the full-stack test suite (`tests/fullstack`, see `Documentation/testing.md`), and the free-boundary chain is wired for `BC_style(2) = 4`. End-to-end free-boundary validation against a known benchmark and the 3-D (field-line) vacuum remain open — treat changes there as higher-risk/experimental, and extend the tests when touching the vacuum.
